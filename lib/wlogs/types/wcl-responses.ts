...
/** DTPS data point (damage taken per second, whole raid, rolling window) */
export interface DTPSPoint {
  timestamp: number; // ms relative to fight start
  dtps: number;
}

/** Echo count data point (active Echo buffs on raid at a point in time) */
export interface EchoCountPoint {
  timestamp: number; // ms relative to fight start
  count: number;
}

export interface CombatantGearItem {
  id: number;
  itemLevel: number;
  quality: number;
}
...
// ── Damage profile types ─────────────────────────────────────

export interface GetDamageProfileParams {
  code: string;
  fightID: number;
  startTime: number;
  endTime: number;
...
/** A single occurrence of an enemy ability hitting the raid */
export interface EnemyAbilityHit {
  timestamp: number; // ms relative to fight start
  totalDamage: number; // total raid damage from this ability at this timestamp
  targetsHit: number;
}

/** An ability used by an enemy (boss/add) */
export interface EnemyAbility {
  ...
  totalDamage: number;
  hitCount: number;
  hits: EnemyAbilityHit[];
}

/** Timeline of one enemy (boss or significant add) */
...
/** A detected damage spike */
export interface DamageSpike {
  startTime: number; // ms relative to fight start
  endTime: number;
  duration: number; // ms
  totalDamage: number;
  dtps: number; // damage per second during spike
  /** Top abilities contributing to this spike */
  topAbilities: Array<{
    abilityGameID: number;
    name: string;
    icon: string;
