...
`;
async function fetchAllPaginated(wcl, query, variables) {
    const allEvents = [];
    let currentStart = variables.startTime;
    ...
    while (true) {
        ...
        if (!events.nextPageTimestamp || events.nextPageTimestamp >= endTime)
            break;
        currentStart = events.nextPageTimestamp;
    }
    return allEvents;
}
export async function buildEssenceTimeline(wcl, code, fightID, startTime, endTime, sourceID, playerName) {
    ...
    const ebStates = [{ timestamp: startTime, stacks: 0 }];
    ...
    function getEBStacksAt(timestamp) {
        let stacks = 0;
...
//# sourceMappingURL=essence-timeline.js.map