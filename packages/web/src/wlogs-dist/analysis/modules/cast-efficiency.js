...
const MOVEMENT_SPELLS = new Set([SPELLS.HOVER, SPELLS.GLIDE]);
export function analyzeCastEfficiency(events, fightDurationMs) {
    const { casts } = events;
    // Only count actual GCD-using casts (cast + empowerend, not begincast/empowerstart)
    const gcdCasts = casts.filter((e) => e.type === "cast" || e.type === "empowerend");
    let healingGCDs = 0;
    let damageGCDs = 0;
    let utilityGCDs = 0;
    let movementGCDs = 0;
    ...
    const longestGap = gaps.reduce((max, g) => (g.durationMs > max.durationMs ? g : max), { startTime: 0, durationMs: 0 });
    // Per-spell breakdown
...
export function formatCastEfficiency(analysis, fightStart) {
    const lines = [];
    ...
    lines.push(`- **GCD split:** ${analysis.healingGCDs} healing / ${analysis.damageGCDs} damage / ${analysis.utilityGCDs} utility`);
    lines.push(`- **Downtime:** ${(analysis.totalDowntimeMs / 1000).toFixed(1)}s (${analysis.downtimePercent.toFixed(1)}%)`);
    lines.push(`- **Fire Breath vs Echo:** ${analysis.fireDamageGCDs} FB / ${analysis.echoGCDs} Echo (ratio: ${analysis.fireToEchoRatio.toFixed(2)})`);
    lines.push("");
    lines.push("### Spell Breakdown");
    for (const s of analysis.spellBreakdown) {
        lines.push(`  - ${s.spell}: ${s.casts} casts (${s.castsPerMinute.toFixed(1)} CPM) [${s.category}]`);
    }
...
//# sourceMappingURL=cast-efficiency.js.map