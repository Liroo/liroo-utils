...
function parseWclUrl(input: string): { code: string | null; fight: number | null; source: number | null } {
  const urlMatch = input.match(/reports\/([a-zA-Z0-9]+)/);
  const code = urlMatch ? urlMatch[1] : /^[a-zA-Z0-9]+$/.test(input.trim()) ? input.trim() : null;
  let fight: number | null = null;
  let source: number | null = null;
  if (code) {
    try {
      const url = new URL(input);
      const f = url.searchParams.get("fight");
      const s = url.searchParams.get("source");
      if (f) fight = Number(f);
      if (s) source = Number(s);
    } catch { /* not a URL */ }
  }
