import type { FightEvents, WCLEvent } from "../event-fetcher.js";
...
export interface EchoAnalysis {
  ...
  echoConsumptions: Record<string, number>; // spell name -> count consumed by
  echoExpired: number;
  echoCastsPerMinute: number;
  echoTargetDistribution: Record<number, number>; // targetID -> echo count
  // Timeline of echo usage windows
  echoWindows: EchoWindow[];
  // Echo active count at key moments (empowered casts)
...
export interface EchoWindow {
  ...
  endTime: number;
  echosApplied: number;
  consumedBy: string | null; // the spell that consumed the echos
}
...
export function analyzeEcho(events: FightEvents, fightDurationMs: number): EchoAnalysis {
  const { casts, buffs } = events;

  // Track Echo casts
  const echoCasts = casts.filter(
    (e) => e.abilityGameID === SPELLS.ECHO && e.type === "cast"
  );

  // Track Echo buff applications and removals
  const echoApply = buffs.filter(
    (e) => e.abilityGameID === SPELLS.ECHO && e.type === "applybuff"
  );
  const echoRemove = buffs.filter(
    (e) => e.abilityGameID === SPELLS.ECHO && e.type === "removebuff"
  );

  // Track TA casts (TA applies echoes automatically)
  const taCasts = casts.filter(
    (e) => e.abilityGameID === SPELLS.TEMPORAL_ANOMALY && e.type === "cast"
  );

  // Track active echos over time for snapshots
  // An echo is "active" between applybuff and removebuff on a target
  const activeEchos = new Map<number, number>(); // targetID -> timestamp applied
  ...
  const echoTimeline: Array<{ timestamp: number; targetID: number; type: "apply" | "remove" }> = [];
  ...
  const empoweredCasts = casts.filter(
    (e) => e.type === "empowerend" &&
    (e.abilityGameID === SPELLS.DREAM_BREATH || e.abilityGameID === SPELLS.SPIRITBLOOM)
  );

  const echoAtEmpowered: EmpoweredEchoSnapshot[] = empoweredCasts.map((emp) => {
  ...
  const targetDist: Record<number, number> = {};
...
function findConsumingCast(
  ...
  for (const cast of casts) {
    if (
      cast.type === "cast" || cast.type === "empowerend"
    ) {
      if (
        Math.abs(cast.timestamp - removalTimestamp) <= BUFFER &&
...
function countActiveEchos(
  timeline: Array<{ timestamp: number; targetID: number; type: "apply" | "remove" }>,
  atTimestamp: number
): { count: number; targets: number[] } {
...
function buildEchoWindows(casts: WCLEvent[]): EchoWindow[] {
  const windows: EchoWindow[] = [];
  let currentWindow: { start: number; count: number } | null = null;
...
export function formatEchoAnalysis(analysis: EchoAnalysis, fightStart: number): string {
  const lines: string[] = [];
  ...
  const sorted = Object.entries(analysis.echoConsumptions).sort(
    ([, a], [, b]) => b - a
  );
  ...
  for (const snap of analysis.echoAtEmpowered) {
    ...
    lines.push(
      `  - ${relTime}s: ${snap.spell} (rank ${snap.empowerLevel}) — **${snap.activeEchos} Echos active**`
    );
  }
  lines.push("");
